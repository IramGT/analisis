  ...
  output$select_sec_1 <- renderUI({
    ...
    output$map <- renderLeaflet({
      tm <- shp_seccion %>% 
        ...
        mutate(Diferencia = `JULIO MENCHACA` - `CAROLINA V`) %>% 
        tm_shape() +
        tm_polygons(col = "Diferencia", id = "seccion", midpoint =50, style = "fixed", breaks = c(-Inf,0,Inf), alpha = .5, palette = c("#3D31F6",  "#AA282D"), title = "Objetivo %", popup.vars = c("Julio M" = "JULIO MENCHACA", "Carolina V" = "CAROLINA V", "Francisco X" = "FRANCISCO X", "José Luis" = "JOSE LUIS"),  legend.show = FALSE) +
